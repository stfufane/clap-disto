...
public:
    ...
    NewBiquadFilter() = default;

    explicit NewBiquadFilter(Type type, double sampleRate,
                             double freq, double q = 0.707, double gainDb = 0.0)
        : mType(type), mSampleRate(sampleRate), mFreq(freq), mQ(q), mGainDb(gainDb) {
        updateCoefficients();
    }

    void setup(Type type, double sampleRate, double freq, double q = 0.707, double gainDb = 0.0) {
        mType = type; mSampleRate = sampleRate; mFreq = freq; mQ = q; mGainDb = gainDb;
        reset();
        updateCoefficients();
    }
    ...
    void setSampleRate(double sampleRate) {
        mSampleRate = sampleRate; updateCoefficients();
    }

    void setType(Type type) { mType = type; updateCoefficients(); }
    void setFreq(double freq) { mFreq = freq; updateCoefficients(); }
    void setQ(double q) { mQ = q; updateCoefficients(); }
    void setGainDb(double gainDb) { mGainDb = gainDb; updateCoefficients(); }
    ...
    void processBuffer(double* samples, std::size_t count) {
        for (std::size_t i = 0; i < count; ++i) samples[i] = process(samples[i]);
    }

    // Process a buffer in-place (float)
    void processBuffer(float* samples, std::size_t count) {
        for (std::size_t i = 0; i < count; ++i) samples[i] = static_cast<float>(process(samples[i]));
    }

    // Returns current coefficients [b0, b1, b2, a1, a2] where a0 == 1
    [[nodiscard]] std::array<double,5> getCoefficients() const {
        return { mB0, mB1, mB2, mA1, mA2 };
    }
    ...
    void updateCoefficients() {
        if (mType == Type::None || mSampleRate <= 0.0) {
            // Bypass
            mB0 = 1.0; mB1 = 0.0; mB2 = 0.0; mA1 = 0.0; mA2 = 0.0;
            return;
        }

        const double nyquist = 0.5 * mSampleRate;
        double f = mFreq;
        if (f <= 0.0) f = 1.0; // prevent zero/negative
        if (f > nyquist * 0.99) f = nyquist * 0.99; // safety clamp under Nyquist
